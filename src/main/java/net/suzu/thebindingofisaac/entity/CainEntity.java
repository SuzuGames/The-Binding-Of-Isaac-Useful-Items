...
public class CainEntity extends TamableAnimal implements RangedAttackMob {
	public CainEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TboiSuzuModEntities.CAIN.get(), world);
	}

	public CainEntity(EntityType<CainEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	public void performRangedAttack(LivingEntity target, float flval) {
		CainTearEntity.shoot(this, target);
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		CainEntity retval = TboiSuzuModEntities.CAIN.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(TboiSuzuModItems.SILVER_DOLLAR.get()).contains(stack.getItem());
	}

	public static void init() {
		SpawnPlacements.register(TboiSuzuModEntities.CAIN.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
