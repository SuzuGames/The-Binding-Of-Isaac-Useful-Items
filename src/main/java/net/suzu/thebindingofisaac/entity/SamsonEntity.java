...
public class SamsonEntity extends TamableAnimal implements RangedAttackMob {
	public SamsonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TboiSuzuModEntities.SAMSON.get(), world);
	}

	public SamsonEntity(EntityType<SamsonEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public void performRangedAttack(LivingEntity target, float flval) {
		SamsonTearEntity.shoot(this, target);
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		SamsonEntity retval = TboiSuzuModEntities.SAMSON.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(TboiSuzuModItems.BLOOD_PENNY.get()).contains(stack.getItem());
	}

	public static void init() {
		SpawnPlacements.register(TboiSuzuModEntities.SAMSON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
