...
public class MagdaleneEntity extends TamableAnimal implements RangedAttackMob {
	public MagdaleneEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TboiSuzuModEntities.MAGDALENE.get(), world);
	}

	public MagdaleneEntity(EntityType<MagdaleneEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 0;
		setNoAi(false);
	...
	public void performRangedAttack(LivingEntity target, float flval) {
		MagdaleneTearEntity.shoot(this, target);
	}

	@Override
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		MagdaleneEntity retval = TboiSuzuModEntities.MAGDALENE.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(TboiSuzuModItems.MAGGYS_FAITH.get()).contains(stack.getItem());
	}

	public static void init() {
		SpawnPlacements.register(TboiSuzuModEntities.MAGDALENE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
