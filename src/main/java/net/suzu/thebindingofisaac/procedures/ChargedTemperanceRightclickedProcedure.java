...
public class ChargedTemperanceRightclickedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			if ((entity instanceof Player _plr ? _plr.getFoodData().getFoodLevel() : 0) >= 1) {
				seleccion = Mth.nextInt(RandomSource.create(), 1, Math.round((entity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) / 2));
				if (!(new Object() {
					public boolean checkGamemode(Entity _ent) {
						if (_ent instanceof ServerPlayer _serverPlayer) {
							return _serverPlayer.gameMode.getGameModeForPlayer() == GameType.CREATIVE;
					...
					if (entity instanceof LivingEntity _entity)
						_entity.setHealth((float) ((entity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) - seleccion * 2));
				}
				for (int index0 = 0; index0 < (int) seleccion; index0++) {
					...
					if (random == 9) {
						if (world instanceof ServerLevel _level) {
							ItemEntity entityToSpawn = new ItemEntity(_level, (x + Mth.nextInt(RandomSource.create(), -4, 4)), (y + 1), (z + Mth.nextInt(RandomSource.create(), -4, 4)), new ItemStack(Items.COPPER_INGOT));
							entityToSpawn.setPickUpDelay(10);
							_level.addFreshEntity(entityToSpawn);
						}
					}
